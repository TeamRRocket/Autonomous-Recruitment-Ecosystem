    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl">
      <div className="flex flex-col gap-4">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Rank #{displayRank}</p>
            <h3 className="text-xl font-semibold text-white mt-1">{candidate?.name || 'Candidate'}</h3>
          </div>
          <div className="text-right">
            <p className="text-xs text-slate-400">Final Score</p>
            <p className="text-3xl font-bold text-emerald-400">{score.toFixed(1)}</p>
          </div>
        </div>
